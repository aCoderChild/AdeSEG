...
class MaskRGBEncoderSO(nn.Module):
    def __init__(self, pretrained=True):
        ...
        resnet = mod_resnet.resnet50(pretrained=pretrained, extra_chan=1)
...
class MaskRGBEncoder(nn.Module):
    def __init__(self, pretrained=True):
        ...
        resnet = mod_resnet.resnet50(pretrained=pretrained, extra_chan=2)
        self.conv1 = resnet.conv1
        self.bn1 = resnet.bn1
        self.relu = resnet.relu  # 1/2, 64
        self.maxpool = resnet.maxpool
...
class RGBEncoder(nn.Module):
    def __init__(self, pretrained=True):
        ...
        resnet = mod_resnet.resnet50(pretrained=pretrained)     #use mod_resnet as backbone
        self.conv1 = resnet.conv1
        self.bn1 = resnet.bn1
        self.relu = resnet.relu  # 1/2, 64
        self.maxpool = resnet.maxpool
...
class Score(nn.Module):
    def __init__(self, input_chan, output_activation="relu"):
    ...
    def forward(self, x):
        ...
        if self.output_activation == "sigmoid":
            ...
            raise ValueError(f"Unknown score output activation: {self.output_activation}")
