...
def update_memory_logits(
    ...
    """Update a single logit state for the controlled memory ablation.

    ``direct`` is the proposed probability-state baseline: every decoded mask
    becomes the next dense prompt. ``fixed`` uses one non-adaptive blend, and
    ``adaptive`` retains the original reliability-gated implementation.
    """
    current_logits = logit(current_probs).astype(np.float32)
    if previous_memory_logits is None or mode == "direct":
        return current_logits
    if mode == "fixed":
        if not 0.0 <= fixed_weight <= 1.0:
            raise ValueError("fixed_weight must be in [0, 1].")
        return (
            fixed_weight * current_logits
            + (1.0 - fixed_weight) * previous_memory_logits.astype(np.float32)
        )
    if mode == "adaptive":
        if reliability is None:
            raise ValueError("adaptive memory update requires reliability.")
        return apply_reliability_gate(current_probs, previous_memory_logits, reliability)
    raise ValueError(f"Unknown memory update mode: {mode!r}.")
