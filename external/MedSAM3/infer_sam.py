...
def resolve_device(device: str) -> torch.device:
    """Choose a stable inference device unless the user requests one.

    SAM3's geometry encoder uses ROI/feature sampling paths that are not stable
    on Apple MPS in this checkout. Keep MPS opt-in so long PolypGen runs do not
    silently produce frame failures when --device is left as auto.
    """
...
class SAM3LoRAInference:
    ...
    def __init__(
        ...
        if base_checkpoint_path is not None and not os.path.exists(base_checkpoint_path):
            raise FileNotFoundError(f"SAM3 base checkpoint not found: {base_checkpoint_path}")
        ...
        self.model = build_sam3_image_model(
            ...
            load_from_HF=base_checkpoint_path is None,
        ...
        load_lora_weights(self.model, weights_path, map_location=self.device)
    ...
    def predict(
        ...
        for query_idx, prompt in enumerate(text_prompts):
            ...
            if num_keep > 0:
                ...
                # torchvision NMS is not consistently implemented on MPS, so run
                # this small postprocess step on CPU while keeping the model on GPU.
                keep_nms = nms(
                    boxes_xyxy.detach().cpu(),
                    kept_scores.detach().cpu(),
                    self.nms_iou_threshold,
                ).to(boxes_xyxy.device)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional local facebook/sam3 base checkpoint path"
    )
    parser.add_argument(
        "--image",
        type=str,
