...
def edt_triton(data: torch.Tensor):
    ...
    if triton is None or not data.is_cuda:
        ...
        import numpy as np
        outputs = []
        for mask in data.detach().cpu().numpy().astype("uint8"):
            outputs.append(cv2.distanceTransform(mask, cv2.DIST_L2, 0))
        return torch.from_numpy(np.stack(outputs)).to(device=data.device, dtype=torch.float32)
    B, H, W = data.shape
