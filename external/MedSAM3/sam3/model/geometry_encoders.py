...
def concat_padded_sequences(seq1, mask1, seq2, mask2, return_index: bool = False):
    ...
    # torch._assert_async is not implemented on Apple MPS. Use a scalar
    # assertion so inference can stay on MPS without CPU fallback side effects.
    assert bool(is_right_padded(mask1).detach().cpu().item())
    assert bool(is_right_padded(mask2).detach().cpu().item())
...
class SequenceGeometryEncoder(nn.Module):
    ...
    def _label_embedding(self, labels):
        labels = labels.long().clamp(0, self.label_embed.num_embeddings - 1)
    ...
    def _encode_points(self, points, points_mask, points_labels, img_feats):
        ...
        type_embed = self._label_embedding(points_labels)
    ...
    def _encode_boxes(self, boxes, boxes_mask, boxes_labels, img_feats):
        ...
        if self.boxes_pool_project is not None:
            ...
            roi_input = boxes_xyxy.float().transpose(0, 1).unbind(0)
            if img_feats.device.type == 'mps':
                sampled = torchvision.ops.roi_align(
                    img_feats.cpu(), [t.cpu() for t in roi_input], self.roi_size
                ).to(img_feats.device)
            else:
                sampled = torchvision.ops.roi_align(
                    img_feats, roi_input, self.roi_size
                )
        ...
        type_embed = self._label_embedding(boxes_labels)
