...
from MedSAM2.sam2.build_sam import build_sam2, build_sam2_video_predictor
...
def yolo_vos_inference(
    ...
    run_log = {
        ...
        "video_logits_dir": video_logits_dir,
    ...
    for reverse in (False, True):
        ...
        for out_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(
            ...
            if video_logits_dir is not None and len(out_obj_ids):
                video_logits[out_frame_idx] = torch.amax(out_mask_logits, dim=0).detach().float().cpu().numpy()
    ...
    if video_logits_dir is not None:
        sequence_logits_dir = os.path.join(video_logits_dir, mask_output_name)
        os.makedirs(sequence_logits_dir, exist_ok=True)
        for frame_index, frame_logits in video_logits.items():
            probability = sigmoid(np.squeeze(frame_logits)).astype(np.float32)
            np.savez_compressed(
                os.path.join(sequence_logits_dir, f"{frame_names[frame_index]}.npz"),
                logits=np.squeeze(frame_logits).astype(np.float32),
                probability=probability,
            )
        run_log["num_saved_video_logits"] = len(video_logits)
...
def main():
    ...
    parser.add_argument(
        "--save_video_logits",
        action="store_true",
        help="Save propagated MedSAM2 video logits and probabilities as per-frame NPZ files.",
    )
    ...
    video_logits_dir = (
        os.path.join(args.output_root, args.method_name, "video_logits")
        if args.save_video_logits
        else None
    )
    ...
    if args.run_eval:
        from utils.eval import run_sequence_evaluation
